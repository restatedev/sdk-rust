...
use restate_sdk::{errors::HandlerError, prelude::{Endpoint, HttpServer}};
use tokio::{io::AsyncBufReadExt, net::TcpListener, task::{self, JoinHandle}};
...
impl TestContainerBuilder {
    ...
    pub fn new() -> TestContainerBuilder {

        TestContainerBuilder { container_name: "docker.io/restatedev/restate".to_string(),
    ...
    pub fn with_container(mut self, container_name:String, container_tag:String) -> TestContainerBuilder {
...
impl TestContainer {
    ...
    pub fn new() -> TestContainer {
        TestContainerBuilder::new().build()
    }
    ...
    async fn serve_endpoint(&mut self, endpoint:Endpoint) -> Result<(), anyhow::Error> {
        ...
        let host_address = format!("127.0.0.1:0");
        let listener = TcpListener::bind(host_address).await.expect("listener can bind");
        ...
        self.endpoint_server_url = Some(format!("http://{}:{}", self.endpoint_server_ip.as_ref().unwrap(), self.endpoint_server_port.as_ref().unwrap()));
        ...
        info!("endpoint server: {}", self.endpoint_server_url.as_ref().unwrap());
    ...
    async fn start_container(&mut self) -> Result<(), anyhow::Error> {
        ...
        self.container = Some(ContainerRequest::from(image)
    ...
    async fn register_endpoint(&mut self) -> Result<(), HandlerError> {
        ...
        info!("registering endpoint server: {}", self.endpoint_server_url.as_ref().unwrap());
        ...
        let deployment_uri:String = format!("http://host.docker.internal:{}/", self.endpoint_server_port.unwrap());
