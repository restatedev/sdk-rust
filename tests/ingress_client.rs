...
#[tokio::test]
async fn send_rejects_malformed_acknowledgements_and_retains_them() {
    async fn run(response: Response<Bytes>) -> ClientError {
        let (client, capture) = client("http://example.test");
        capture.respond(response);
        expect_error(
            client
    ...
    let error = run(invocation_response(
        StatusCode::ACCEPTED,
        "malformed",
    ...
    .await;
    assert!(matches!(error, ClientError::Protocol { .. }));
    assert_preserved_response(error, StatusCode::ACCEPTED, malformed_body);

    let missing_header_body = br#"{"invocationId":"missing-header","status":"Accepted"}"#;
    let mut missing_header = raw_response(StatusCode::ACCEPTED, missing_header_body.as_slice());
    missing_header
        .headers_mut()
        .insert("x-test-response", HeaderValue::from_static("retained"));
    let error = run(missing_header).await;
    assert!(matches!(error, ClientError::Protocol { .. }));
    assert_preserved_response(error, StatusCode::ACCEPTED, missing_header_body);

    let missing_body_id = br#"{"status":"Accepted"}"#;
    let error = run(invocation_response(
        StatusCode::ACCEPTED,
        "header-id",
        missing_body_id.as_slice(),
    ))
    .await;
    assert!(matches!(error, ClientError::Protocol { .. }));
    assert_preserved_response(error, StatusCode::ACCEPTED, missing_body_id);

    let empty_body_id = br#"{"invocationId":"","status":"Accepted"}"#;
    let error = run(invocation_response(
        StatusCode::ACCEPTED,
        "header-id",
        empty_body_id.as_slice(),
    ))
    .await;
    assert!(matches!(error, ClientError::Protocol { .. }));
    assert_preserved_response(error, StatusCode::ACCEPTED, empty_body_id);

    let mismatched_id = br#"{"invocationId":"body-id","status":"Accepted"}"#;
    let error = run(invocation_response(
        StatusCode::ACCEPTED,
        "header-id",
        mismatched_id.as_slice(),
    ))
    .await;
    assert!(matches!(error, ClientError::Protocol { .. }));
    assert_preserved_response(error, StatusCode::ACCEPTED, mismatched_id);

    let unknown_status = br#"{"invocationId":"header-id","status":"FutureStatus"}"#;
    let error = run(invocation_response(
        StatusCode::ACCEPTED,
        "header-id",
        unknown_status.as_slice(),
    ))
    .await;
    assert!(matches!(error, ClientError::Protocol { .. }));
    assert_preserved_response(error, StatusCode::ACCEPTED, unknown_status);
}
