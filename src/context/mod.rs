...
pub trait ContextExtensions<'ctx>: private::SealedContext<'ctx> {
    ...
    fn extension<T: Send + Sync + 'static>(&self) -> &'ctx T {
        self.try_extension::<T>().unwrap_or_else(|| {
            panic!(
                "Handler `{}` requested extension of type `{}`, but it was not registered. \
                 Register it with `.extension(..)` on the service or endpoint builder.",
...
pub trait ContextClient<'ctx>: private::SealedContext<'ctx> {
    ...
    /// Create a service client. The client is generated by the [`service!`](crate::prelude::service) macro, named after the service suffixed with `Client`.
    ///
    /// ```rust,no_run
    /// # use std::time::Duration;
    /// # use restate_sdk::prelude::*;
    ///
    /// #[restate_sdk::handler]
    /// async fn handle(ctx: Context<'_>) -> HandlerResult<()> { Ok(()) }
    ///
    /// service!(MyService: { handle });
    ///
    ...
    /// Create an object client. The client is generated by the [`object!`](crate::prelude::object) macro, named after the object suffixed with `Client`.
    ///
    /// ```rust,no_run
    /// # use std::time::Duration;
    /// # use restate_sdk::prelude::*;
    ///
    /// #[restate_sdk::handler]
    /// async fn handle(ctx: ObjectContext<'_>) -> HandlerResult<()> { Ok(()) }
    ///
    /// object!(MyObject: { handle });
    ///
    /// # async fn handler(ctx: Context<'_>) {
    ...
    /// Create a workflow client. The client is generated by the [`workflow!`](crate::prelude::workflow) macro, named after the workflow suffixed with `Client`.
    ///
    /// ```rust,no_run
    /// # use std::time::Duration;
    /// # use restate_sdk::prelude::*;
    ///
    /// #[restate_sdk::handler]
    /// async fn run(ctx: WorkflowContext<'_>) -> HandlerResult<()> { Ok(()) }
    ///
    ...
    ///
    /// // Submit the workflow (the `run` handler runs at most once per id)
    /// let result = client.run(()).call().await;
    ///
    /// // Just send the request, don't wait the response
    /// client.run(()).send();
    ///
    /// // Schedule the request to be executed later
    /// client.run(()).send_after(Duration::from_secs(60));
    /// # }
    /// ```
