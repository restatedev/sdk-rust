...
async fn test_container() {
    ...
    // let mut test_container = TestContainer::new().start(endpoint).await.unwrap();
    ...
    let test_container = TestContainer::builder()
                                ...
                                .with_container("docker.io/restatedev/restate".to_string(),  "latest".to_string())
                                .build()
                                .start(endpoint).await.unwrap();
    ...
    assert_eq!(resposne.status(), StatusCode::OK);

    info!("/MyService/my_handler response: {:?}", resposne.text().await.unwrap());
